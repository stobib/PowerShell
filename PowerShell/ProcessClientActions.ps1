Function RunClientAction{[CmdletBinding()]param(
    [Parameter(Position=0,Mandatory=$True,HelpMessage="Provide server names",ValueFromPipeline=$true)][string[]]$Computers,
    [ValidateSet('HardwareInventory','SoftwareInventory','DataDiscoveryRecord','FileCollection','IDMIFCollection','ClientMachineAuthentication','MachinePolicyAssignmentsRequest','MachinePolicyEvaluation',
    'RefreshDefaultMPTask','LSRefreshLocationsTask','LSTimeoutRefreshTask','PolicyAgentRequestAssignment','PolicyAgentEvaluateAssignment','SoftwareMeteringGeneratingUsageReport','SourceUpdateMessage',
    'Clearingproxysettingscache','MachinePolicyAgentCleanup','UserPolicyAgentCleanup','PolicyAgentValidateMachinePolicy','PolicyAgentValidateUserPolicy','RefreshingcertificatesinADonMP','PeerDPStatusreporting',
    'PeerDPPendingpackagecheckschedule','SUMUpdatesinstallschedule','HardwareInventoryCollectionCycle','SoftwareInventoryCollectionCycle','DiscoveryDataCollectionCycle','FileCollectionCycle','IDMIFCollectionCycle',
    'SoftwareMeteringUsageReportCycle','WindowsInstallerSourceListUpdateCycle','SoftwareUpdatesAssignmentsEvaluationCycle','BranchDistributionPointMaintenanceTask','SendUnsentStateMessage',
    'StateSystempolicycachecleanout','ScanbyUpdateSource','UpdateStorePolicy','Statesystempolicybulksendhigh','Statesystempolicybulksendlow','Applicationmanagerpolicyaction','Applicationmanageruserpolicyaction',
    'Applicationmanagerglobalevaluationaction','Powermanagementstartsummarizer','Endpointdeploymentreevaluate','EndpointAMpolicyreevaluate','Externaleventdetection')]
    [string[]]$ClientAction
)
    $ActionResults=@()
    Try{
        ForEach($ComputerName In $Computers){
            $ComputerName=($ComputerName+".vdi.utshare.local")
...
#RunClientAction -Computername $env:ComputerName -ClientAction 'HardwareInventory','SoftwareInventory','ScanbyUpdateSource','SoftwareUpdatesAssignmentsEvaluationCycle'
RunClientAction -Computers (Get-Content .\VMExport\InputFile.txt) -ClientAction 'HardwareInventory','SoftwareInventory','ScanbyUpdateSource','SoftwareUpdatesAssignmentsEvaluationCycle'