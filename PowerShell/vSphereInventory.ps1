            ...
            ForEach($VM In $VMLabel){
                ...
                If(($VM.Name-eq$VMGuest)-or($VM-eq$VMGuest)){
                    ...
                    ForEach($Folder In $ExcludedFolders){
                        If($Folder-eq$VMStatus.Folder.Name){
                            $Excluded=$True;Break
                        }
                    }
                    If($Excluded){
                        ("****`tBypassing ["+$FQDN+"] because it's in the excluded folder: ["+$VMStatus.Folder.Name+"].`t****")|Out-File $LogFile -Append
                    ...
                    If(($ByPass-eq$False)-and(($FQDN-ne"")-or(!($FQDN)))){
                        Write-Host ("`tVerifying a running and healthy state for ["+$FQDN+"].")
...
#                                            $Results=Connect-Mstsc -ComputerName $FQDN -Credential $SecureCredentials
        ...
        $Message+=("The attachment: ["+$ServerList+"] is a list of ["+$VMCount+"] systems that were processed from vSphere inventory.  ")
        $Message+=("The second attachment are the results from each VMGuest of the ["+$VMProcessed+"] VMs processed from the ["+$LogName+"] file.  ")
        $AttachmentList=@($ProcessList,$LogFile)
        If($EXcount-gt0){
            $AttachmentList+=($ExcludedFile)
            $Message+=("The third attachment ["+$ExcludedName+"] is a list of ["+$EXcount+"] VMGuest systems that have been marked as excluded.  ")
        }Else{
            $Message+=("The attachment ["+$ExcludedName+"] was not included because there were ["+$EXcount+"] VMGuest systems that were excluded.  ")
        }
        If($POCount-gt0){
            $AttachmentList+=($PoweredOffFile)
            $Message+=("The last attachment ["+$PoweredOffName+"] is a list of ["+$POCount+"] VMGuest systems that are currently powered off and won't be processed.")
        }Else{
        ...
        Send-MailMessage -From "<$($Sender)>" -To $SendTo -Subject ("Summary of VMWare systems from Site-"+$Site+"") -Body $Message -Attachments $AttachmentList -SmtpServer $MailServer
        If($FailedToConnect){
