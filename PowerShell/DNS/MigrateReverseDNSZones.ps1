...
ForEach($GC In $AllDCs){
    ...
    Set-Variable -Name ReverseZones -Value @(Get-DnsServerZone -ComputerName $DnsServer|Where-Object{$_.ZoneName -like ("*.10.in-addr.arpa")})
    ForEach($ZoneData In $ReverseZones){
