...
$DefaultParameterValues=@{"LogFileNames"="Uninstall"}
...
Set-Variable -Name ByPass -Value $true
Set-Variable -Name ComputerList -Value @(Get-ADComputer -Filter {(OperatingSystem -like "Win*10*")-and(Enabled -eq $true)} -Properties Name)
ForEach($Computer In $ComputerList){
    Switch($Computer){
        {$_.Name-like"win10aar*"}{$ByPass=$true;Break}
        {$_.Name-like"win10ada*"}{$ByPass=$true;Break}
        {$_.Name-like"win10aep*"}{$ByPass=$true;Break}
        {$_.Name-like"win10arg*"}{$ByPass=$true;Break}
        {$_.Name-like"win10apb*"}{$ByPass=$true;Break}
        {$_.Name-like"win10asa*"}{$ByPass=$true;Break}
        {$_.Name-like"win10aty*"}{$ByPass=$true;Break}
        Default{$ByPass=$false;Break}
    }
    If($ByPass){
    }Else{
        $InstallLocation=$null
        $Message=("Beginning to process "+$Computer.Name)
        $Message|Out-File -FilePath $LogFN1 -Append
        Write-Host $Message
        $DriveNames=(Get-PSDrive -PSProvider FileSystem).Name
        ForEach($Drive In $DriveNames){
            $RootDrive=@((Get-ChildItem -Path ("\\"+$Computer.Name+"\"+$Drive+"$") -Depth 2 -Recurse -ErrorAction SilentlyContinue|Where-Object {$_.Extension -eq ".properties"}).FullName)
            $UsersFolder=@((Get-ChildItem -Path ("\\"+$Computer.Name+"\"+$Drive+"$\users") -Depth 4 -Recurse -ErrorAction SilentlyContinue|Where-Object {$_.Extension -eq ".properties"}).FullName)
            If($UsersFolder){
                $FolderList=@($RootDrive,$UsersFolder)
            }Else{
                $FolderList=@($RootDrive)
            }
            ForEach($CurrentFolder In $FolderList){
                If(Test-Path -Path $CurrentFolder){
                    $FileContents=Get-Content -Path $CurrentFolder
                    ForEach($Line In $FileContents){
                        If($Line-like"installlocation*"){
                            $InstallLocation=($Line.Split("="))[1]
                            $InstallLocation=$InstallLocation.Replace("\:","$")
                            $InstallLocation=("\\"+$Computer.Name+"\"+$InstallLocation.Replace("\\","\"))
                            If(!(Test-Path -Path $InstallLocation)){
                                $InstallLocation=Split-Path $CurrentFolder -Parent
                            }
                        }
                        If($Line-like"productversion*"){
                            $ProductVersion=($Line.Split("="))[1]
                            $ProductVersion=($Line.Split("."))[1]
                        }
                    }
                    If($ProductVersion-le"54"){
                        $Message=("`tRemoving directory "+$InstallLocation+" from: "+$Computer.Name)
                        $Message|Out-File -FilePath $LogFN1 -Append
                        Write-Host $Message
                        Remove-Item $InstallLocation -Recurse -Force # -Verbose 4>&1|Out-File -FilePath $LogFN1 -Append
                    }
                }
            }
        }
        $Message=("Finished processing "+$Computer.Name+"`n`n")
        $Message|Out-File -FilePath $LogFN1 -Append
        Write-Host $Message
    }
}
