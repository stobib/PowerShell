...
$filepath = Read-OpenFileDialog -WindowTitle "Select Text File with VM's to Yum Update" -InitialDirectory 'C:\' -Filter "Text files (*.txt)|*.txt"
...
$gcred = Get-Credential -Message "For Guest Machines to Update (all must have the same password)"

foreach($computer in $computers){

    #$update = "yum check duplicates"
    #$update = "yum clean all && yum update -y && init 6"
    $update = "yum clean all && yum update -y && init 0"
    #$update = "uptime"

    Invoke-VMScript -vm $computer -ScriptText $update -GuestCredential $gcred -ScriptType Bash -RunAsync
    #-RunAsync
