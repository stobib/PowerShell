...
ForEach($computer in $computers){
    ...
    $sysPresent = Get-VMGuest -VM $computer

    if($sysPresent){
        Write-Host "System $computer exists in VMWare." | Out-File C:\pwscriptoutputs\existencecheck\test1.txt 
    }
    else{
        $pingable = Test-Connection -ComputerName $computer
            if($pingable){
                Write-Host "System $computer exists but not in VMWare." |Out-File C:\pwscriptoutputs\existencecheck\test1.txt
            }
            else{
                Write-Host "Unable to ping $computer or find this system in VMWare." |Out-File C:\pwscriptoutputs\existencecheck\test1.txt
            }
    }    
