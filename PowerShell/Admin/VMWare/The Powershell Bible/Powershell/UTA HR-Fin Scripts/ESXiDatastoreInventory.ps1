#THIS SCRIPT ONLY RUNS ON POWERSHELL VERSION 3 AND ABOVE!!!
...
$filepath = Read-OpenFileDialog -WindowTitle "Select Text File With List of ESXi Hosts" -InitialDirectory 'C:\' -Filter "Text files (*.txt)|*.txt"
...
foreach ($singleViserver in $vihosts){
    ...
    $HostDatastoreList = Get-Datastore -VMHost $singleViserver 
    $esxHostinfo = get-vmhost -name $singleViserver | %{(Get-View $_.ID).Config.Product}
    $OSBuildInfo = $esxHostinfo.FullName
    

    ForEach($DS in $HostDatastoreList){
               
    

    $entry = @{            
                Hostname = $singleViserver              
                Version  = $OSBuildInfo                 
                Datastore = $DS 
    }                           


    New-Object PSObject -Property $entry | Export-CSV -Append -path “c:\pwscriptoutputs\datastore\vmdatastoreinfo-$date.csv” –NoTypeInformation
