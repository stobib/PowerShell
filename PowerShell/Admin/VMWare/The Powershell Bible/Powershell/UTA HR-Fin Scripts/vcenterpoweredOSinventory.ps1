...
#Get-Cluster -name "ARDC PROD-DEV" |Get-VM | Select Name, PowerState, @{N="IPAddress"; E={$_.Guest.IPAddress[0]}},@{N="Running OS";E={$_.Guest.OsFullName}}  | sort Name | Export-Csv -path “c:\pwscriptoutputs\inventory\vminventory-$date.csv” –NoTypeInformation
#Get-VMGuest | sort Name | Export-Csv -path “c:\pwscriptoutputs\inventory\vminventory-$date.csv” –NoTypeInformation
...
#Get-VM | Select Name, PowerState, NumCPU, MemoryMB, ProvisionedSpaceGB, UsedSpaceGB,  @{N=”Guest OS”;E={Get-VMGuest -VM $_}},@{N="Running OS";E={$_.Guest.OsFullName}} | sort Name | Export-Csv -path “c:\pwscriptoutputs\inventory\vminventory-$date.csv” –NoTypeInformation

Get-VM | Select Name, PowerState, @{N="Tools Status";E={$_.Guest.Extensiondata.ToolsVersionStatus}},@{N="IPAddress"; E={$_.Guest.IPAddress[0]}},@{N="Running OS";E={$_.Guest.OsFullName}},@{N="HostName";E={$_.Guest.HostName}}  | sort Name | Export-Csv -path “c:\pwscriptoutputs\inventory\vminventory-$date.csv” –NoTypeInformation