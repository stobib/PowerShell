...
# OR ANY INCIDENTAL, CONSEQUENTIAL, DIRECT, INDIRECT, OR SPECIAL DAMAGES, ARISING OUT
# OF THIS OR ANY OTHER AGREEMENT RELATING TO THE SAMPLE CODE.
...
# Variables #
#############
$filelocation="f:\scripts\healthcheck.htm"
...
$ClusterName= "aec-cloud"
$metrics = "cpu.usage.average", "mem.usage.average"
 
$cluster =  Get-Cluster | where-Object {$_.name -like $ClusterName}
Get-Cluster -Name $cluster.Name | Get-Stat -Realtime -Stat $metrics -IntervalMins 1 -MaxSamples 1 | ForEach-Object {$_}
 

foreach($objItem in $colItems) {$objItem.MetricId + "," + $objItem.Value} | 
...
#################################
$vm | where { $_ | get-cddrive | where { $_.ConnectionState.Connected -eq "true" } } | Select Name | ConvertTo-Html –title "CDROMs connected" –body "<H2>CDROMs connected.</H2>" -head "<link rel='stylesheet' href='style.css' type='text/css' />"|Out-File -Append $filelocation

#########################################
# VMware floppy drives connected to VMs #
#########################################
$vm | where { $_ | get-floppydrive | where { $_.ConnectionState.Connected -eq "true" } } | select Name |ConvertTo-Html –title "Floppy drives connected" –body "<H2>Floppy drives connected.</H2>" -head "<link rel='stylesheet' href='style.css' type='text/css' />" |Out-File -Append $filelocation
