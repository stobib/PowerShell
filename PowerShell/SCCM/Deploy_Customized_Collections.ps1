...
ForEach($Folder In $FolderStructure){
    ...
    If($Folder[0]-ne0){
        $ContainerNodeID=(Get-Wmiobject -ComputerName $SiteServer -Namespace root\sms\site_$SiteCode -Class SMS_ObjectContainernode -Filter "ObjectType = 5000 AND NAme = '$ParentContainer'").ContainerNodeID
    }
    ...
    Try{
        ...
        If($ContainerData-ne$null){
            ...
            Write-Host -ForegroundColor Cyan ("Folder: """+$Folder[1]+""" already exists on Site Server "+$SiteServer+".")
        }
    }Catch{
...
; E={@("select distinct SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System where SMS_R_System.SMSAssignedSites = ""$($SiteCode)""")}},@{L="RuleName"
...
; E={@("select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System where SMS_R_System.SMSAssignedSites = ""$($SiteCode)""")}},@{L="RuleName"
...
If($Overwrite-eq1){
    ...
    ForEach($Collection In $Collections){
        ...
        If(!($Collection.Folder-eq"root")){
            $FolderPath=(Get-FolderPath -SiteServer $SiteServer -SiteCode $SiteCode -FolderName $Collection.Folder)
            Try{
